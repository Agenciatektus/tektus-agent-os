...
### 🔄 Sincronização Universal (Triple-Sync)
Para garantir que Antigravity, Claude e Cursor utilizem sempre as mesmas diretrizes, implementamos o **Triple-Sync**:
- **Fonte da Verdade**: `.agents/rules/*.md`
- **Destino Claude**: `.claude/rules/*.md` (Mirror Identitário)
- **Destino Cursor**: `.cursor/rules/*.mdc` (Conversão Automática)

> [!IMPORTANT]
> Nunca edite as regras diretamente nas pastas do Claude ou Cursor. Qualquer alteração deve ser feita na `.agents/rules` e processada pelo script de manutenção.

#### Como Sincronizar:
O script `skills/update_memory.py` realiza o sync automaticamente.
- **Manual**: `python skills/update_memory.py "Ajuste na regra X" "Descritivo da mudança"`
- **Automático**: Configurado via Hook `on_agent_turn_end` no Antigravity.

---

### 🛡️ Sandboxing e Segurança Terminais
